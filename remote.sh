...
case $key in
    ...
    -c|--copy)
    SCP_FILE="$2"
    ####zip files to scp multiple files
...
####Create list of host from file (-l option)
HOST_ARRAY=()
if [ ! -z $HOST_LIST ]; then
...
####Create list of single remote host (-h option)
if [ ! -z $REMOTE_HOST ]; then
	if [ -z $HOST_LIST ]; then
...
####Loop through the list of host and perform the action
x=1
for i in "${HOST_ARRAY[@]}"
do
	...
	####ssh interactive or command mode
	if [ ! -z "$COMMAND" ]; then
		if [ ! -z $ROOT ]; then
...
####Sleep to avoid script alerts
sleep 1

####echo . to show activity
echo -n '.'
done
echo
...
if [ ! -z "$COMMAND" ]; then
	####loop through pids to tail output 
	for ((i=1; i<$x; i++)); do
		...
		tail_pids[${i}]=$!
		####disown to supress output when killing the tail process
		disown 
	done
